   ...
   /*  let hashtags = (!props.text) ? "" : props.text.split(' ').filter(str => str.startsWith('#')).join(' ');
    ...
    return (
        <Box width="100%" className="post-captions">
            ...
            <Typography className={"caption-text " + classes.caption} variant="subtitle1">{props.caption}</Typography>
            <Typography className={"hastag-text " + classes.hashtags} variant="subtitle2">{props.hashtags}</Typography>
        </Box>
    );
